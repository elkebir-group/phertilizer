...
class Branching_split():
    ...
    def __init__(self,
        ...
        #set data
        self.like0 = like0
        self.like1  = like1_marg
        self.like1_dict= like1
        self.var = var
        self.total = total
        ...
        self.cnn_hmm = cnn_hmm
   
        self.snv_bin_mapping = snv_bin_mapping
        self.states = ["gain", "loss", "neutral"]

        self.bin_mapping = {m : self.snv_bin_mapping.loc[m] for m in self.snv_bin_mapping.index}
    ...
    def calc_tmb(self, cells, muts):
        ...
        mut_var_count_by_cell = np.count_nonzero(self.var[np.ix_(cells, muts)],axis=1).reshape(-1,1)
        mut_total_reads_by_cell = np.count_nonzero(self.total[np.ix_(cells, muts)],axis=1).reshape(-1,1)
    ...
    def create_affinity(self,  mutsA, mutsB, mutsC, cells, weight=0.5):
        ...
        if len(mutsA) > 0 and len(mutsB) > 0:
            ...
            mut_features = impute_mut_features(copy_dist_mat, mut_features, cells)
    ...
    def mut_assignment_with_cna(self,  curr_tree):
        m = self.like0.shape[1]
        cellsA = curr_tree.get_tip_cells(1)
        cellsB = curr_tree.get_tip_cells(2)
        all_cells = curr_tree.get_all_cells()
        zA = curr_tree.event_by_node(m, cellsA, 1, self.snv_bin_mapping)
        zB = curr_tree.event_by_node(m, cellsB, 2, self.snv_bin_mapping)
        z = curr_tree.event_matrix(m,  self.snv_bin_mapping)
               
        for s in self.states:
            mutsA= self.like0[cellsB, :].sum(axis=0) 
            mutsB = self.like0[cellsA,:].sum(axis=0) 
            mutsC = np.zeros(shape=m, dtype=float)
            for s in self.states:   
                like1_mat = self.like1_dict[s]
                mutsA += np.ma.array(like1_mat[cellsA,:], mask=zA !=s).sum(axis=0)
                mutsB  += np.ma.array(like1_mat[cellsB,:], mask=zB !=s).sum(axis=0)
                mutsC += np.ma.array(like1_mat[all_cells,:], mask=z !=s).sum(axis=0)
    ...
    def cluster_cells(self, mutsA, mutsB, mutsC, cells):
        ...
        if mut_features is not None:
            avg_muts_ma = mut_features[mut_features.index.isin(cells1)].mean(axis=0)
    ...
    def mut_assignment(self, cellsA, cellsB, eA, eB):
        ...
        mutsA_cand = like1[np.ix_(cellsA, self.muts)].sum(axis=0) + self.like0[np.ix_(cellsB, self.muts)].sum(axis=0)
        mutsB_cand =  self.like0[np.ix_(cellsA, self.muts)].sum(axis=0) + like1[np.ix_(cellsB, self.muts)].sum(axis=0)
        mutsC_cand =  like1[np.ix_(cellsA, self.muts)].sum(axis=0) + like1[np.ix_(cellsB, self.muts)].sum(axis=0)
    ...
    def assign_cna_events(self, cellsA, cellsB):
            ...
            eventsA = self.cnn_hmm.run(cellsA)

            eventsB = self.cnn_hmm.run(cellsB)


            return eventsA, eventsB


    def run_without_cna(self, w):
        mutsA, mutsB, mutsC= self.random_init(self.muts)
        oldA = np.empty(shape=0, dtype=int)
        for j in range(self.iterations):
            cellsA, cellsB = self.cluster_cells(mutsA, mutsB, mutsC,weight=w)
        ...
        if len(cellsA) >0  and len(cellsB) >0:
            cand_tree = BranchingTree(cellsA, cellsB, mutsA, mutsB, mutsC, eA, eB)

            self.cand_trees.insert(cand_tree)


    def run_with_cna(self):
        ...
        oldA = np.empty(shape=0, dtype=int)
        for j in range(self.iterations):
            cellsA, cellsB, stats = self.cluster_cells(mutsA, mutsB, mutsC,cells)
            # all_na_cells = np.union1d(all_na_cells, na_cells)
            if len(cellsB) ==0 or len(cellsA)==0 or np.array_equal(np.sort(cellsA), np.sort(oldA)):
                break
            else:
                oldA = cellsA
            ...
            curr_tree = BranchingTree(cellsA, cellsB, mutsA, mutsB, mutsC, eA, eB, eC=None)

            mutsA, mutsB, mutsC  = self.mut_assignment(cellsA, cellsB, eA=None, eB=None)
        ...
        if (len(cellsA) > self.lamb and len(cellsB) > 1)  or (len(cellsB) > self.lamb and len(cellsA) > 1):
             ...
             if check_stats(stats, self.jump_percentage, self.spectral_gap,self.npass):
                eA, eB = self.assign_cna_events(cellsA, cellsB)
                cand_tree = BranchingTree(cellsA, cellsB, mutsA, mutsB, mutsC, eA, eB, eC=None)
                print(cand_tree)
                self.cand_trees.insert(cand_tree)
    ...
    def sprout(self, include_cna=True):
        ...
        if include_cna:
            self.run = self.run_with_cna
        else:
            self.run = self.run_without_cna
        ...
        if self.cand_trees.has_trees():
            best_branching_tree, _ = self.cand_trees.find_best_tree(self.like0, self.like1_dict, self.snv_bin_mapping, self.cnn_hmm)
