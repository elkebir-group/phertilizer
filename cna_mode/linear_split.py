...
class Linear_split():
    ...
    def run_helper_with_cna(self, cells, muts, weight, p):
        if len(cells)  > 2*self.lamb:
            ...
            if best_tree is not None:
                ...
                self.run_helper_with_cna(best_tree.get_tip_cells(0), muts, weight, p)
    ...
    def sprout(self, seed, infer_cna_events=True):
    
        self.norm_like_list = []
        self.snv_bin_mapping = self.snv_bin_mapping.filter(items=seed.muts)

        self.cand_splits = ClonalTreeList()


        self.run_helper = self.run_helper_with_cna

  
        muts = seed.muts
        cells = seed.cells
        self.run_helper(cells, muts, 0.5, p=0.5)
