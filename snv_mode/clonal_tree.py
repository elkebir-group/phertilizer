...
class ClonalTree:
    ...
    def rdr_likelihood(self, cnn_hmm):
        ...
        for n in self.tree.nodes():
            cells = self.get_tip_cells(n)
            if len(cells) > 0:
                node_total, cell_series = self.rdr_likelihood_by_node(
                    n, cnn_hmm)
                total += node_total
    ...
    def get_parent_cells(self, node):
        ...
        while True:
            ...
            parent_cells = np.concatenate(
                [self.cell_mapping[parent][k] for k in self.cell_mapping[parent]])
    ...
    def event_matrix(self, m, bin_mapping):
        ...
        # n x m character matrix z_ij = state where state in {+, -, 0}
        return z
    ...
    def compute_likelihood(self, like0, like1, snv_bin_mapping=None, cnn_hmm=None):
        ...
        for n in self.cell_mapping:
            if len(self.get_tip_cells(n)) > 0:
                varlike = self.compute_variant_likelihood_by_node_without_events(
                    n, like0, like1)
                self.node_likelihood[n] = {
                    "total": varlike, "variant": varlike, "bin": 0}
    ...
    def cell_genotype_vector(self, ncells, node, desc_dict):
        ...
        if len(desc_dict[node]) > 0:
            desc_cells = np.concatenate(
                [self.get_tip_cells(child) for child in desc_dict[node]])
    ...
    def get_cell_genotypes(self, ncells):
        desc_dict = {n: nx.descendants(self.tree, n)
    ...
    def find_low_likelihood_cells(self, like0, like1, snv_bin_mapping, perc, cna_hmm=None):
        ...
        for n in self.cell_mapping:
            cells = self.get_tip_cells(n)
            if len(cells) > 0:
                cell_like_list.append(self.mut_likelihood_by_node_with_events(
                    n, like0, like1, snv_bin_mapping, ax=1))
                all_cells.append(cells)
        cell_total_like = pd.Series(np.concatenate(
            cell_like_list), index=np.concatenate(all_cells)).sort_index()
    ...
    def find_low_likelihood_muts(self, like0, like1, snv_bin_mapping, perc):
        ...
        for n in self.cell_mapping:
            if len(self.get_tip_cells(n)) > 0:
                mut_like_vec += self.mut_likelihood_by_node_with_events(
                    n, like0, like1, snv_bin_mapping, ax=0)
    ...
    def post_process(self, like0, like1, snv_bin_mapping,  lamb, tau, min_loss, cna_hmm, perc, iterations):
        cells = self.find_low_likelihood_cells(
            like0, like1, snv_bin_mapping, perc, cna_hmm)
        muts = self.find_low_likelihood_muts(
            like0, like1, snv_bin_mapping, perc)
        # logging.info(f"Post Process: Cells: {len(cells)} Muts: {len(muts)}")

        if not self.include_cna:
            self.post_process_mapping(
                cells, muts,  lamb, tau, min_loss, like0, like1, iterations)
        else:
            self.post_process_mapping_with_cna(
                cells, muts, lamb, tau, min_loss, like0, like1, snv_bin_mapping, cna_hmm, iterations)
    ...
    def eliminate_illegal_linear_nodes(self):
        ...
        for n in self.cell_mapping:
            if len(self.get_tip_cells(n)) == 0 and self.tree.out_degree(n) == 1 and len(self.get_tip_muts(n)) > 0:
                ...
                self.mut_mapping[parent] = np.union1d(
                    self.mut_mapping[parent], muts)
        return count

    def post_proc(self, lamb, tau, min_loss, like0, like1, snv_bin_mapping, cna_hmm, perc, iterations):
        print(self)
        illegal_nodes, pruned_cells, prune_muts = self.prune_nodes(
            lamb, tau, min_loss)
        post_cells = self.find_low_likelihood_cells(
            like0, like1, snv_bin_mapping, perc, cna_hmm)
        post_muts = self.find_low_likelihood_muts(
            like0, like1, snv_bin_mapping, perc)
    ...
    def prune_and_reassign(self, lamb, tau, min_loss, like0, like1, snv_bin_mapping, cna_hmm):
        ...
        if len(illegal_nodes) and (len(pruned_cells) > 0 or len(prune_muts) > 0):
            self.post_process_mapping_with_cna(
                pruned_cells, prune_muts, lamb, tau, min_loss, like0, like1, snv_bin_mapping, cna_hmm, iterations=1)
    ...
    def post_process_mapping_with_cna(self, cells, muts, lamb, tau, min_loss, like0, like1, snv_bin_mapping, cna_hmm, iterations):
        ...
        black_list_nodes = [x for x in list(self.tree.nodes()) if len(np.concatenate(
            [self.cell_mapping[x][k] for k in self.cell_mapping[x]])) == 0 and self.tree.out_degree(x) > 0]
        ...
        for i in range(iterations):
            ...
            print(self)
            if len(muts) > 0:
                self.mut_post_processing_with_cna(
                    muts, like0, like1, snv_bin_mapping)

            if len(cells) > 0:
                self.cell_post_processing_with_cna(
                    cells, like0, like1, snv_bin_mapping, cna_hmm, black_list_nodes)

                self.event_post_processing(cna_hmm)

            post_like = self.compute_likelihood(
                like0, like1, snv_bin_mapping, cna_hmm)
            pre_like = post_like
    ...
    def cell_post_processing_with_cna(self, cells,  like0, like1, snv_bin_mapping, cna_hmm, black_list_nodes=[]):
        ...
        like_list = []
        nodes_with_cells = [n for n in self.cell_mapping if len(
            self.get_tip_cells(n) > 0)]
        ...
        for i, n in enumerate(nodes_evaluated):
            new_cell[n] = np.array(
                cells[np.argwhere(cell_assignment == i)]).reshape(-1)
    ...
    def mut_post_processing_with_cna(self, muts, like0, like1, snv_bin_mapping):
        ...
        if len(self.mut_loss_mapping) > 0:
            all_lost_muts = np.concatenate(
                [self.mut_loss_mapping[k] for k in self.mut_loss_mapping])
        ...
        for n in cell_geno_dict:
            ...
            mut_likelihoods = np.multiply(
                1-cg, like0).sum(axis=0) + np.multiply(cg, like1).sum(axis=0)
        ...
        for i, n in enumerate(nodes_evaluated):
            mut_mapping[n] = np.array(
                muts[np.argwhere(mut_assignment == i)]).reshape(-1).astype(int)
    ...
    def update_mut_dict(self, muts, new_assign):
        for n in self.mut_mapping:
            ...
            self.mut_mapping[n] = np.concatenate(
                [self.mut_mapping[n], new_assign[n]]).astype(int)
    ...
    def update_cell_dict(self, cells, new_assign):
        for n in self.cell_mapping:
            ...
            if n in new_assign:
                self.cell_mapping[n] = {0: np.concatenate(
                    [remaining_cells, new_assign[n]]).astype(int)}
...
class LinearTree(ClonalTree):
    ...
    def is_valid(self, lamb, tau, min_loss):
...
class LossTree(ClonalTree):
    ...
    def get_seeds(self, lamb, tau, ancestral_muts):
        ...
        anc_muts = np.sort(np.setdiff1d(
            ancestral_muts, self.mut_loss_mapping[0]))
        seed_list.append(Seed(cellsB, mutsB, anc_muts))
