        ...
        for right in bins:
            ...
            if right == left + 1:
                ...
                if bin2chrom is not None:
                    start_chrom = bin2chrom.loc[bin2chrom["start"]
                                                == leftmost, :]
                    end_chrom = bin2chrom.loc[abs(
                        bin2chrom["end"] - left) <= 1, :]
                    if len(start_chrom) > 0 and len(end_chrom) > 0:
                        if all(start_chrom.index == end_chrom.index):
                            s.append(
                                f"{start_chrom['chrom'].item()}{start_chrom['arm'].item()}")
...
class DrawClonalTree:
    def __init__(self, clonal_tree, bin2chrom=None):
        ...
        # self.node_events = clonal_tree.node_events
        ...
        self.include_subclusters = any(
            [len(self.cm[n]) > 1 for n in self.nodes])

        self.cell_count = {n: len(np.concatenate(
            [self.cm[n][k] for k in self.cm[n]])) for n in self.nodes}
        ...
        for n in self.nodes:
            ...
            if len(self.cm[n]) > 1 and self.include_subclusters:
                ...
                for s in self.cm[n]:
                    ...
                    self.tree.add_node(
                        index, label=f"Cells {n}_{s}: {len(self.cm[n][s])}")

                    self.tree.add_subgraph(
                        subgraph, name=f"cluster_{n}", label=f"Cluster {n}")
