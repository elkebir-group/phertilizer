...
def find_cells_with_no_reads(mat, cells, muts):
    total_reads_by_cell = np.count_nonzero(
        mat[np.ix_(cells, muts)], axis=1).reshape(-1)

    return cells[total_reads_by_cell == 0]


def find_muts_with_no_reads(mat, cells, muts):
    total_reads_by_mut = np.count_nonzero(
        mat[np.ix_(cells, muts)], axis=0).reshape(-1)
...
def impute_mut_features(c_mat, mut_features, cells):
    ...
    for c in cells_to_impute:
        ...
        replacement_cell_index = cell_series[cell_series ==
                                             replacement_cell_id].index[0]
...
def normalizedMinCut(W, index):
    ...
    print(
        f"Jump Percentage: {jump_percentage} First Eig Val: {eig_vals[0]} First Gap: {first_gap} Largest Gap: {largest_gap} Number of Clusters:{spectral_k}")
    ...
    labels = AgglomerativeClustering(
        n_clusters=2, affinity="euclidean", linkage="ward").fit_predict(vecs[:, 1].reshape(-1, 1))
